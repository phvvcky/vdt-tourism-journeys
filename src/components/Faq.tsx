      ...
      <div className="container-vdt grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        >
          {t.faq.items.map((item, i) => (
            <details
              key={item.q}
              className="group px-5 py-4 transition-colors hover:bg-secondary/40"
              open={i === 0}
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-heading text-base font-semibold text-card-foreground">
                {item.q}
                <ChevronDown className="h-4 w-4 shrink-0 text-vdt-red transition-transform duration-300 group-open:rotate-180" />
              </summary>
